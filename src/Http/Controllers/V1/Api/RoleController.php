...
class RoleController extends ApiController
{
    ...
    public function index(RoleIndexRequest $request)
    {
        try {
            ...
            )->getCollection(asResourceCollection: true,asResponseData: true,attributes: config('callmeaf-role.resources.index.attributes'));
            return apiResponse([
                'roles' => $roles,
    ...
    {
        try {
            $role = $this->roleService->setModel($role)->getModel(asResource: true,attributes: config('callmeaf-role.resources.show.attributes'),relations: config('callmeaf-role.resources.show.relations'));
            return apiResponse([
                'role' => $role,
            ],__('callmeaf-base::v1.successful_loaded'));
        } catch (\Exception $exception) {
