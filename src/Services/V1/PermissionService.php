...
use Callmeaf\Permission\Services\V1\Contracts\PermissionServiceInterface;
...
class PermissionService extends BaseService implements PermissionServiceInterface
{
    public function __construct(?Builder $query = null, ?Model $model = null, ?Collection $collection = null, ?JsonResource $resource = null, ?ResourceCollection $resourceCollection = null, array $defaultData = [],?string $searcher = null)
    {
        ...
        $this->query = app(config('callmeaf-permission.model'))->query();
        $this->resource = config('callmeaf-permission.model_resource');
        $this->resourceCollection = config('callmeaf-permission.model_resource_collection');
        $this->defaultData = config('callmeaf-permission.default_values');
        $this->searcher = config('callmeaf-permission.searcher');
    }
